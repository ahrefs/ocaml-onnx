...
OrtStatus* cuda_provider_options_update_cuda_provider_options(OrtCUDAProviderOptionsV2 *cuda_options, char** keys, char** values, size_t n_keys) {
  return current_ort()->UpdateCUDAProviderOptions(cuda_options, (const char* const*)keys, (const char* const*)values, n_keys);
}
